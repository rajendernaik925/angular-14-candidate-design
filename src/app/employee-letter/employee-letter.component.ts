...
export class EmployeeLetterComponent implements OnInit {
  ...
  confirmationletterprivileges:any;
  constructor(
  ...
  ngOnInit(): void {
    ...
    let m= decodeURIComponent(window.atob(localStorage.getItem('confirmationletterprivileges')));
    this.confirmationletterprivileges = JSON.parse(m);
    //console.log(this.incrementletterprivileges.VIEW);
  ...
  ConfirmationshowFormModal(){
    this.authService.getConfirmationPDF(this.userData.user.empID, "confirmation").subscribe((responseMessage) => {
          let file = new Blob([responseMessage], { type: 'application/pdf' });
          var fileURL = URL.createObjectURL(file);
