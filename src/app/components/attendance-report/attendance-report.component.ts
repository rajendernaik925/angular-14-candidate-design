...
@Component({
  ...
  styleUrls: ['./attendance-report.component.sass']
})
export class AttendanceReportComponent implements OnInit {
 ...
 public selectedLevel:any;
  ...
  ngOnInit(): void { 
    ...
    if (this.refreshService.checkForRefresh('attendance')) {
      this.refreshService.refreshData('attendance');
    } else {
  ...
  //  console.log(this.userData.user.sublocation)
    ...
    this.flexiPolicy = fileBasepath + "Policydocuments/flexi_working_hours_policy.pdf#toolbar=0&view=FitH";
    this.mumFlexiPolicy = fileBasepath + "Policydocuments/mumbai.pdf#toolbar=0&view=FitH";
    ...
    this.authService.attendance(finalParams).subscribe(
      (res) => { 
        ...
        for(let i=0; i<this.inOutData.length; i++){ 
          if(this.inOutData[i].DISPLAYNAME ==='ABSENT' ){ 
            this.lopCount++;           
          }
        }
        // console.log("LOP______________" + this.lopCount);
